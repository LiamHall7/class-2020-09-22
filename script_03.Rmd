...
Group by all duplicates
Eliminate inapplicable data
Understand that students add and drop classes and enrollments may change
Compare enrollment numbers between Sept 1 and Sept 22
Check in with professors whose classes have seen large decline in enrollment

After we have a model, Temperance suggests we should:

Recognize that students' mentalities are hard to capture through data
External circumstances could be the reason for dropped classes
...
```{r}
sep_old %>% 
  left_join(sep_new, suffix = c(new, new))
```
